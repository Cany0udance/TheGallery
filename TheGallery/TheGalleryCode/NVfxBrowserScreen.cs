...
public partial class NVfxBrowserScreen : NSubmenu
{
    ...
    private NAudioBrowserTab _audioBrowser;
    private NCreatureAnimBrowserTab _creatureBrowser;
    ...
    private void BuildCreaturesTab(Vector2 screenSize, float contentTop, float availableHeight)
    {
        ...
        _creatureBrowser = new NCreatureAnimBrowserTab();
        _creatureBrowser.Initialize(_creaturesTab);
    }
    ...
    public override void OnSubmenuClosed()
    {
        ...
        ClearCurrentVfx();
        _audioBrowser?.CleanupAndRestore();
        _creatureBrowser?.CleanupAndRestore();
    }
    
    public override void _Process(double delta)
    {
        base._Process(delta);
        _creatureBrowser?.Process(delta);
    }

    public override void _Input(InputEvent @event)
    {
        base._Input(@event);
        _creatureBrowser?.HandleInput(@event);
    }
...
private void BuildBgButtons(Control parent, float previewX)
    ...
    private void BuildAudioTab(Vector2 screenSize, float contentTop, float availableHeight)
    {
        ...
        _audioBrowser = new NAudioBrowserTab();
        _audioBrowser.Initialize(_audioTab);
    }
