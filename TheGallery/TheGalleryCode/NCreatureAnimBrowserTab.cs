...
namespace TheGallery.TheGalleryCode;

public class NCreatureAnimBrowserTab
{
    // ── Layout constants ──────────────────────────────────────────────
    ...
    public void Initialize(Control parent)
    {
        try
        {
            ...
            var screenSize = _root.GetViewportRect().Size;
            ...
            float availableHeight = parent.Size.Y > 0 ? parent.Size.Y : screenSize.Y;
            float availableWidth = parent.Size.X > 0 ? parent.Size.X : screenSize.X;
    ...
    {
        if (_root == null || !_root.IsVisibleInTree()) return;
    ...
    {
        if (_root == null || !_root.IsVisibleInTree() || _previewArea == null) return;
 
        var mouse = _root.GetGlobalMousePosition();
    ...
    private void BuildListPanel(float listWidth, float availableHeight)
    {
        ...
        _root.AddChild(_entryCountLabel);
    ...
    private void BuildPreviewPanel(float previewX, float previewWidth, float availableHeight)
    {
        ...
        _root.AddChild(_currentCreatureLabel);
        ...
        _root.AddChild(previewPanelBorder);
    ...
    private void BuildAnimationControls(float previewX, float previewWidth)
    {
        ...
        _root.AddChild(_playPauseButton);
        ...
        _root.AddChild(_track1Dropdown);
    }
    ...
    private void BuildBoneControls(float previewX, float previewWidth)
    {
        ...
        _root.AddChild(_showBonesButton);
        ...
        _root.AddChild(_showAllBoneNamesCheckbox);
    }
    ...
    {
        var entryMargin = new MarginContainer { SizeFlagsHorizontal = Control.SizeFlags.ExpandFill };
